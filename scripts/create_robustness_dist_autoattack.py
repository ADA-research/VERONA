...
from robustness_experiment_box.verification_module.attack_estimation_module import AttackEstimationModule  # noqa: E501
from robustness_experiment_box.verification_module.attacks.auto_attack_wrapper import AutoAttackWrapper  # noqa: E501
...
def create_distribution(experiment_repository: ExperimentRepository,
    ...
    for network in network_list:
        ...
        for data_point in sampled_data:
            verification_context = (
                ...
                .create_verification_context(network,
                                             data_point,
                                             property_generator))

            epsilon_value_result = (
...
def main():
    ...
    torch_dataset = torchvision.datasets.MNIST(root="./data", train=True,
                                               download=True,
    ...
    (experiment_repository
     .save_configuration(dict(
                            experiment_name=experiment_name,
                            experiment_repository_path=str(
                                experiment_repository_path),
                            network_folder=str(network_folder),
                            dataset=str(dataset),
                            epsilon_list=[str(x) for x in epsilon_list])))
    create_distribution(experiment_repository, dataset, dataset_sampler,
                        epsilon_value_estimator, property_generator)
