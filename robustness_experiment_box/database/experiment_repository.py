...
class ExperimentRepository:
    ...
    def __init__(self, base_path: Path, network_folder: Path) -> None:
        ...
        with the base path and network folder.
    ...
    def create_verification_context(
        self, network: Network, data_point: DataPoint,
        property_generator: PropertyGenerator
    ) -> VerificationContext:
        """
        Create a verification context for the given network, data point,
        and property generator.
        ...
        """
        tmp_path = (
            self.get_tmp_path() / f"{self.get_file_name(network.path)}" /
            f"image_{data_point.id}"
        )
        return VerificationContext(
            network, data_point, tmp_path, property_generator
        )
    ...
    def get_per_epsilon_result_df(self) -> pd.DataFrame:
        ...
        for network_folder in network_folders:
            ...
            for image_folder in images_folders:
                t_df = pd.read_csv(
                    image_folder / per_epsilon_result_df_name, index_col=0
                )
    ...
    def save_per_epsilon_result_df(self) -> None:
        ...
        per_epsilon_result_df.to_csv(
            self.get_results_path() / PER_EPSILON_RESULT_CSV_NAME
        )
    ...
    def save_plots(self) -> None:
        ...
        hist_figure.savefig(
            self.get_results_path() / "hist_figure.png", bbox_inches='tight'
        )

        boxplot = report_creator.create_box_figure()
        boxplot.savefig(
            self.get_results_path() / "boxplot.png", bbox_inches='tight'
        )

        kde_figure = report_creator.create_kde_figure()
        kde_figure.savefig(
            self.get_results_path() / "kde_plot.png", bbox_inches='tight'
        )

        ecdf_figure = report_creator.create_ecdf_figure()
        ecdf_figure.savefig(
            self.get_results_path() / "ecdf_plot.png", bbox_inches='tight'
        )

    def save_verification_context_to_yaml(
        self, file_path: Path, verification_context: VerificationContext
    ) -> Path:
        """
    ...
    def load_verification_context_from_yaml(self,
                                            file_path: Path
                                            ) -> VerificationContext:
        """
