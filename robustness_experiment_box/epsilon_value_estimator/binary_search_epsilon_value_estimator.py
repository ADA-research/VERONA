...
class BinarySearchEpsilonValueEstimator(EpsilonValueEstimator):
    ...
    def compute_epsilon_value(self,
        ...
        highest_unsat_value, smallest_sat_value = self.binary_search(
        ...
        logger.info(
            ...
            f"{verification_context.get_dict_for_epsilon_result()}, "
            f"epsilon_result: {epsilon_value_result.epsilon}"
        )
        return epsilon_value_result

    def get_highest_unsat(self,
        ...
        """
        highest_unsat = None
        unsat_results = [x.result for x in epsilon_status_list
                         if x.result == VerificationResult.UNSAT]
        if len(unsat_results) > 0:
            highest_unsat = max(
                [index for index, x in enumerate(epsilon_status_list)
                 if x.result == VerificationResult.UNSAT]
            )

        highest_unsat_value = (
            epsilon_status_list[highest_unsat].value
            if highest_unsat is not None
            else 0
        )

        return highest_unsat_value

    def get_smallest_sat(self,
                         epsilon_status_list: list[EpsilonStatus]
                         ) -> float:
        """
        ...
        sat_results = [x.result for x in epsilon_status_list
        ...
        smallest_sat_value = (
            epsilon_status_list[smallest_sat].value
            if smallest_sat is not None
            else max_epsilon_value
        )
    ...
    def binary_search(self,
        ...
        """
        Perform binary search to find the highest UNSAT
        ...
        if len(epsilon_status_list) == 1:
            outcome = self.verifier.verify(
                ...
                epsilon_status_list[0].value)
            result = outcome.result
            epsilon_status_list[0].time = outcome.took
            epsilon_status_list[0].result = result
            logger.debug(
                f'current epsilon value: {epsilon_status_list[0].result}, '
                f'took: {epsilon_status_list[0].time}'
            )
            verification_context.save_result(epsilon_status_list[0])
            if result == VerificationResult.UNSAT:
                return (epsilon_status_list[0].value,
                        self.get_smallest_sat(epsilon_status_list))
            else:
        ...
        while first <= last:
            ...
            if not epsilon_status_list[midpoint].result:
                ...
                logger.debug(
                    f'current epsilon value: '
                    f'{epsilon_status_list[midpoint].result}, '
                    f'took: {epsilon_status_list[midpoint].time}'
                )

            if (epsilon_status_list[midpoint].result ==
                    VerificationResult.UNSAT):
                first = midpoint + 1
            elif (epsilon_status_list[midpoint].result ==
        ...
        logger.debug(
            f"epsilon status list: "
            f"{[(x.value, x.result, x.time) for x in epsilon_status_list]}"
        )
