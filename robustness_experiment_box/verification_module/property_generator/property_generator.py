...
class PropertyGenerator(ABC):
    ...
    Methods:
        create_vnnlib_property(image: np.array, image_class: int,
        epsilon: float) -> VNNLibProperty:
            Abstract method to create a VNNLib property based on the given
            image, its class, and epsilon value.
        get_dict_for_epsilon_result() -> dict:
            Abstract method to get a dictionary representation of
            the epsilon result.
        to_dict():
