...
class AttackEstimationModule(VerificationModule):
    ...
    def verify(self, verification_context: VerificationContext,
        ...
        if isinstance(verification_context.property_generator,
            ...
            data_on_device = (verification_context.data_point.data.clone()
                              .detach().to(device))
            perturbed_data = self.attack.execute(torch_model, data_on_device,
                                                 target_on_device, epsilon)
            ...
            if final_pred == target:
                return CompleteVerificationData(
                    result=VerificationResult.UNSAT, took=duration)
            else:
                return CompleteVerificationData(
                    result=VerificationResult.SAT, took=duration)
        else:
            raise NotImplementedError(
                "Currently, only one 2 any verification is implemented "
