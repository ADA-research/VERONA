...
class AutoVerifyModule(VerificationModule):
    """
    A module for automatically verifying the robustness of a model
    using a specified verifier.
    """

    def __init__(self, verifier: autoverify.verifier.verifier.Verifier,
                 timeout: float, config: Path = None) -> None:
        """
        Initialize the AutoVerifyModule with a specific verifier, timeout
        and optional configuration.
        Args:
            verifier (Verifier): The verifier to be used
    ...
    def verify(self, verification_context: VerificationContext,
        ...
        """
        image = (verification_context.data_point.data
                 .reshape(-1).detach().numpy())
        vnnlib_property = (verification_context.property_generator
                           .create_vnnlib_property(
                               image,
                               verification_context.data_point.label,
                               epsilon))
        ...
        if self.config:
            result = self.verifier.verify_property(
                verification_context.network.path, vnnlib_property.path,
                timeout=self.timeout, config=self.config)
        else:
            result = self.verifier.verify_property(
                verification_context.network.path,
                vnnlib_property.path, timeout=self.timeout)
...
def parse_counter_example(result: Ok) -> np.ndarray:
    """
    ...
    """
    string_list_without_sat = [
        x for x in result.unwrap().counter_example.split("\n")
        if "sat" not in x
    ]
    numbers = [
        x.replace("(", "").replace(")", "")
        for x in string_list_without_sat if "Y" not in x
    ]
    counter_example_array = np.array([
        float(re.sub(r'X_\d*', '', x).strip())
        for x in numbers])
...
def parse_counter_example_label(result: Ok) -> int:
    ...
    """
    string_list_without_sat = [x for x in
                               result.unwrap().counter_example.split("\n")
                               if "sat" not in x]
    numbers = [x.replace("(", "").replace(")", "") for x in
               string_list_without_sat if "X" not in x]
    counter_example_array = np.array([float(re.sub(r'Y_\d*', '', x).strip())
                                      for x in numbers])
