...
class DummyAutoAttack:
    def __init__(
        self, model, *, attacks=None, device="cpu", eps=0.3, norm="Linf", seed=None, version="standard", verbose=False
    ):
        self.model = model
        self.device = device
        self.epsilon = eps
...
def test_autoattack_verbose_assignment(monkeypatch, attack_wrapper, model, data, target, verbose_value):
    monkeypatch.setattr("ada_verona.verification_module.attacks.auto_attack_wrapper.AutoAttack", DummyAutoAttack)
