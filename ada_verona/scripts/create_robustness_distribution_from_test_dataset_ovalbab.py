...
experiment_name = "auto_verify_ovalbab_test"
timeout = 10
...
# Check if auto-verify is available and OvalBab verifier exists
if not ada_verona.HAS_AUTO_VERIFY:
    raise RuntimeError("Auto-verify is not available. Please install auto-verify package.")

if "ovalbab" not in ada_verona.AUTO_VERIFY_VERIFIERS:
    raise RuntimeError(
        f"OvalBab verifier is not available. Available verifiers: {ada_verona.AUTO_VERIFY_VERIFIERS}"
    )

# Create verifier using plugin architecture
verifier = ada_verona.create_auto_verify_verifier("ovalbab", timeout=timeout)
if verifier is None:
    raise RuntimeError("Failed to create OvalBab verifier through plugin system.")
