...
# Check if auto-verify is available and Verinet verifier exists
if not ada_verona.HAS_AUTO_VERIFY:
    raise RuntimeError("Auto-verify is not available. Please install auto-verify package.")

if "verinet" not in ada_verona.AUTO_VERIFY_VERIFIERS:
    raise RuntimeError(
        f"Verinet verifier is not available. Available verifiers: {ada_verona.AUTO_VERIFY_VERIFIERS}"
    )

# Create verifier using plugin architecture
verifier = ada_verona.create_auto_verify_verifier("verinet", timeout=timeout)
if verifier is None:
    raise RuntimeError("Failed to create Verinet verifier through plugin system.")
